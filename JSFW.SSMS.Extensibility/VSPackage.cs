    ...
    public sealed class VSPackage : Package
    {
            ...
            foreach (EnvDTE.Command cmd in _applicationObject.Commands)
            {
                switch (cmd.Name)
                {
                    ...
                    case "Tools.ColumnAlign":
                        cmd.Bindings = new object[] { "전역::CTRL+Q,CTRL+R" };
                        break;
                    case "Tools.개체찾기":
                        cmd.Bindings = new object[] { "전역::CTRL+Q,CTRL+F" };
                        break;                    
                } 
            }
        }
        ...
        private void HeaderFrozenColumnColumnButton_Click(CommandBarButton Ctrl, ref bool CancelDefault)
        {
            ...
            if (grid.FirstScrollableColumn == 1)
            {
                ...
                if (1 <= col && (col + 1) < grid.GridColumnsInfo.Count)
                {
                    ...
                    //grid.GetGridColumnInfo(grid.FirstScrollableColumn).SetBackgroundColor(System.Drawing.Color.Coral);
                }
                else
                {
            ...
            //grid.GridColumnsInfo[grid.FirstScrollableColumn].SetBackgroundColor(System.Drawing.Color.Yellow);
            //grid.UpdateGrid();
            grid.Refresh();
        }
        ...
        private void HeaderColumnsAlignCopyButton_Click(CommandBarButton Ctrl, ref bool CancelDefault)
        {
            ...
            for (int col = 1; col < grid.ColumnsNumber; col++)
            {
                ...
                text += $"{hd.TrimEnd()}:{hd.TrimEnd()}{Environment.NewLine}";// + "\t";
            }
